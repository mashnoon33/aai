import { z } from "zod";
import { loadCsvData, type Trial } from "@/lib/load-csv";
import { createTRPCRouter, publicProcedure } from "@/server/api/trpc";

const trials = loadCsvData();
...
const sortTrials = (trials: Trial[], sortField: keyof Trial, sortDirection: 'asc' | 'desc') => {
    ...
    .query(({ input }) => {
      ...
      if (input.studyStatus) {
        filteredTrials = filteredTrials.filter(trial => 
          trial['Study Status'].toLowerCase().includes(input.studyStatus!.toLowerCase())
        );
      }
      if (input.conditions) {
        filteredTrials = filteredTrials.filter(trial => 
          trial.Conditions.toLowerCase().includes(input.conditions!.toLowerCase())
        );
      }
      if (input.studyType) {
        filteredTrials = filteredTrials.filter(trial => 
          trial['Study Type'].toLowerCase().includes(input.studyType!.toLowerCase())
        );
      }
      
      // Apply sorting
      filteredTrials = sortTrials(filteredTrials, input.sortField, input.sortDirection);
      
      // Calculate pagination
      const totalItems = filteredTrials.length;
      const totalPages = Math.ceil(totalItems / input.pageSize);
      const startIndex = (input.page - 1) * input.pageSize;
      const paginatedTrials = filteredTrials.slice(startIndex, startIndex + input.pageSize);
